...
unsigned int chk_selinux_enabled;
unsigned int chk_selinux_enforcing;
...
static int __init byeselinux_init(void)
{
	...
	chk_selinux_enabled = _selinux_is_enabled();
	...
	*_selinux_enabled = 0;
	...
	{
		chk_selinux_enforcing = *_selinux_enforcing;
		pr_info("byeselinux: old selinux_enforcing: %u\n", chk_selinux_enforcing);
...
{
	if(_selinux_enabled != NULL)
		*_selinux_enabled = chk_selinux_enabled;

	if(_selinux_enforcing != NULL)
		*_selinux_enforcing = chk_selinux_enforcing;
